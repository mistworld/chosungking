  ...
  async applyUpdate(roomId, update) {
    ...
    if (update.action === "submit_word" && state.gameMode === "turn") {
      ...
      if (state.turnStartTime) {
        const turnTimeLimit = state.isFirstTurn ? 9e3 : 6e3;
...
async function handleRooms(env) {
  ...
  try {
    ...
    const roomIdSet = /* @__PURE__ */ new Set();
    ...
    const recentRoomPromises = Array.from(recentRoomIds).filter((id) => !list.keys.some((k) => k.name === id)).map((id) => env.ROOM_LIST.get(id, "json"));
...
async function handleJoinRoom(request, env) {
  ...
  if (playerName) {
    const duplicateName = roomData.players.find(
      (p) => p.name && p.name.toLowerCase() === playerName.toLowerCase() && p.id !== playerId
    );
    if (duplicateName) {
      return jsonResponse({
...
// .wrangler/tmp/bundle-VUqs0B/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VUqs0B/middleware-loader.entry.ts
