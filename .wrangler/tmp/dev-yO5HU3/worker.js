  ...
  try {
    ...
    for (const key of list.keys) {
      try {
        ...
        const allPlayers = Array.isArray(roomData.players) ? roomData.players : [];
        const lastSeen = roomData.lastSeen || {};
        const ACTIVE_TIMEOUT = 60 * 1e3;
        const activePlayers = allPlayers.filter((p) => {
          const seen = lastSeen[p.id];
          if (!seen) return true;
          return now - seen < ACTIVE_TIMEOUT;
        });
...
// .wrangler/tmp/bundle-TTRXLR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TTRXLR/middleware-loader.entry.ts
