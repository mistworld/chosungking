...
async function handleRooms(env) {
  ...
  try {
    ...
    console.log(`[rooms] \uC804\uCCB4 \uBC29 \uAC1C\uC218: ${list.keys.length}`);
    if (list.keys.length > 0) {
      console.log(`[rooms] \uBC29 \uD0A4 \uBAA9\uB85D: ${list.keys.slice(0, 10).map((k) => k.name).join(", ")}${list.keys.length > 10 ? "..." : ""}`);
    }
    const now = Date.now();
    ...
    for (let i = 0; i < list.keys.length; i++) {
      ...
      try {
        ...
        if (!isNewRoom && roomData.lastSeen && typeof roomData.lastSeen === "object" && players.length > 0) {
          ...
          console.log(`[rooms] \uBC29 ${roomId}: \uC624\uB798\uB41C \uBC29, activePlayers=${activePlayers.length}, playerCount=${playerCount}`);
        } else {
...
// .wrangler/tmp/bundle-a16hYa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-a16hYa/middleware-loader.entry.ts
