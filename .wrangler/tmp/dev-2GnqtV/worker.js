    ...
    for (const key of list.keys) {
      try {
        ...
        if (meta && meta.id) {
          ...
          if (roomData && !roomData.gameStarted && now - roomData.createdAt < ONE_HOUR) {
            rooms.push({
...
async function handleLeaveRoom(request, env) {
  ...
  await env.ROOM_LIST.put(roomId, JSON.stringify(roomData));
  return jsonResponse({ success: true, remainingPlayers: roomData.players.length });
}
...
async function handleGameState(request, env) {
  ...
  if (request.method === "GET") {
    ...
    const doResponse = await stub2.fetch(request);
    let doState = null;
    if (doResponse.ok) {
    ...
    if (roomData.scores) {
      doState.scores = { ...doState.scores, ...roomData.scores };
    }
    if (roomData.playerWords) {
      doState.playerWords = { ...doState.playerWords, ...roomData.playerWords };
    }
    console.log(`[game-state] GET ${roomId}: players=${doState.players.length}, gameStarted=${doState.gameStarted}`);
...
// .wrangler/tmp/bundle-XJQWma/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XJQWma/middleware-loader.entry.ts
