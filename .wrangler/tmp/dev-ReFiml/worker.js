    ...
    for (let i = 0; i < list.keys.length; i++) {
      ...
      try {
        ...
        if (roomData.lastSeen && typeof roomData.lastSeen === "object") {
          const activePlayers = Array.isArray(roomData.players) ? roomData.players.filter((p) => {
            const last = roomData.lastSeen[p.id];
            return typeof last === "number" && now - last < STALE_PLAYER_TIMEOUT;
          }) : [];
          playerCount = activePlayers.length;
        } else {
          playerCount = Array.isArray(roomData.players) ? roomData.players.length : 0;
        }
        if (playerCount <= 0 && Array.isArray(roomData.players) && roomData.players.length > 0) {
...
// .wrangler/tmp/bundle-PIDizu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PIDizu/middleware-loader.entry.ts
