    ...
    for (let i = 0; i < list.keys.length; i++) {
      ...
      try {
        ...
        if (roomData.lastSeen && typeof roomData.lastSeen === "object" && players.length > 0) {
          ...
          if (activePlayers.length === 0 && now - createdAt > 10 * 1e3) {
            playerCount = 0;
          } else {
            playerCount = Math.max(activePlayers.length, players.length);
          }
        }
...
// .wrangler/tmp/bundle-o530CG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-o530CG/middleware-loader.entry.ts
