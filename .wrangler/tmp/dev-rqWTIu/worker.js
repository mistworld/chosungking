...
async function handleRooms(env) {
  ...
  const STALE_PLAYER_TIMEOUT = 5 * 1e3;
  try {
    if (!env.ROOM_LIST) {
    ...
    for (let i = 0; i < list.keys.length; i++) {
      ...
      try {
        ...
        const isNewRoom = age < 60 * 1e3;
        const players = Array.isArray(roomData.players) ? roomData.players : [];
        let playerCount = players.length;
        console.log(`[rooms] \uBC29 ${roomId}: players=${players.length}, age=${Math.floor(age / 1e3)}\uCD08, isNewRoom=${isNewRoom}, lastSeen=${roomData.lastSeen ? "\uC788\uC74C" : "\uC5C6\uC74C"}`);
        if (!isNewRoom && roomData.lastSeen && typeof roomData.lastSeen === "object" && players.length > 0) {
          const activePlayers = players.filter((p) => {
            const last = roomData.lastSeen[p.id];
            const isActive = typeof last === "number" && now - last < STALE_PLAYER_TIMEOUT;
            if (!isActive) {
              console.log(`[rooms] \uD50C\uB808\uC774\uC5B4 ${p.id} \uBE44\uD65C\uC131: last=${last ? Math.floor((now - last) / 1e3) + "\uCD08 \uC804" : "\uC5C6\uC74C"}`);
            }
            return isActive;
          });
          playerCount = activePlayers.length;
          console.log(`[rooms] \uBC29 ${roomId}: activePlayers=${activePlayers.length}, playerCount=${playerCount}`);
        } else {
          console.log(`[rooms] \uBC29 ${roomId}: \uC0C8 \uBC29\uC774\uAC70\uB098 lastSeen \uC5C6\uC74C, playerCount=${playerCount} (players.length \uC0AC\uC6A9)`);
        }
...
// .wrangler/tmp/bundle-ldvZ1Y/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ldvZ1Y/middleware-loader.entry.ts
